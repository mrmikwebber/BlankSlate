...
export default function CollapsibleTable() {
  ...
  const handleInputChange = (categoryIndex, itemIndex, value) => {
    ...
    const currentlyAssigned = data.reduce((sum, category) => {
      return sum + category.items.reduce((itemSum, item) => itemSum + item.assigned, 0);
    }, 0);
          ...
          {data.map((group, categoryIndex) => (
            <Fragment key={group.category}>
              ...
              >
                ...
                <td className="p-2 border">{formatToUSD(group.items.reduce((sum, item) => sum + item.assigned, 0))}</td>
                <td className="p-2 border">{formatToUSD(group.items.reduce((sum, item) => sum + item.activity, 0))}</td>
                <td className="p-2 border">{formatToUSD(group.items.reduce((sum, item) => sum + item.available, 0))}</td>
              </tr>
              ...
              {openCategories[group.category] &&
                group.items.map((item, itemIndex) => (
                  <tr key={itemIndex} className="border-t">
                    ...
                    <td className="p-2 border">{formatToUSD(item.available)}</td>
                  </tr>
                ))}
            </Fragment>
          ))}
