...
export async function PATCH(req: Request) {
  ...
  // Upsert the shadow assignment — never touches budget_assignments (real).
  ...
  if (upsertError) {
    console.error("[budget/global-assign] upsert error:", upsertError);
    return NextResponse.json({ error: "Failed to save global assignment" }, { status: 500 });
  }
